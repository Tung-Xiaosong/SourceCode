...
for (auto& point :point_cloud.points) {
    time_tempx.push_back(point.x());
    time_tempy.push_back(point.y());
}
int i_time = 0;
for (auto& point :point_cloud.points) {
    point.x() =  time_tempx[point_cloud.points.size()-1-i_time];
    point.y() =  time_tempy[point_cloud.points.size()-1-i_time];
    i_time++;
}
...
std::unique_ptr<nav_msgs::OccupancyGrid> CreateOccupancyGridMsg(
  ...
  for (int y = height - 1; y >= 0; --y) {
    for (int x = 0; x < width; ++x) {
      ...
      if(value_temp > 0 && value_temp < 40) value_temp = 0;
      if(value_temp > 55) value_temp = 100; 
      occupancy_grid->data.push_back(value_temp);
    }
  }
