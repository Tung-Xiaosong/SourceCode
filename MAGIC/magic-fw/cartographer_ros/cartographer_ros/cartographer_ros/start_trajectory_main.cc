...
DEFINE_string(configuration_directory, "/home/yhs/catkin_ws/src/cartographer_ros/cartographer_ros/configuration_files",
              "First directory in which configuration files are searched, "
              "second is always the Cartographer installation to allow "
              "including files from there.");
DEFINE_string(configuration_basename, "dgt_001mini_loc.lua",
...
void init_pose_callback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg)
{
	...
	std::string translation = std::to_string(x) + "," + std::to_string(y) + "," + "0";
	std::string rotation =    std::to_string(0) + ",0,0";

	initial_pose = "{to_trajectory_id=0,relative_pose={translation={" + translation + "},rotation={" + rotation + "}}}";
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber _pose_init_sub = nh.subscribe("/initialpose", 1, cartographer_ros::init_pose_callback);
