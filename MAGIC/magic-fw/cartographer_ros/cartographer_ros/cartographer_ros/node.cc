...
void Node::PublishTrajectoryStates(const ::ros::WallTimerEvent& timer_event) {
  ...
  for (const auto& entry : map_builder_bridge_.GetTrajectoryStates()) {
    ...
    //by xwqf add
    tf2::Quaternion q;
    tf2::fromMsg(imu_msg_.orientation, q);
...
//    LOG(INFO) << roll << "  "  << pitch  << " "  << yaw;
    ...
    if (trajectory_state.published_to_tracking != nullptr) {
      if (trajectory_state.trajectory_options.provide_odom_frame) {
        ...
        //by xwqf start
        tf2::Quaternion q1;
        tf2::fromMsg(stamped_transform.transform.rotation, q1);
        double roll1, pitch1, yaw1;
        tf2::Matrix3x3(q1).getRPY(roll1, pitch1, yaw1);

        tf2::Quaternion imu_link_rotation;
        imu_link_rotation.setRPY(roll, pitch, 0.0);
        tf2::Quaternion yaw_rotation;
        yaw_rotation.setRPY(0.0, 0.0, yaw1);
        imu_link_rotation = yaw_rotation * imu_link_rotation;
        stamped_transform.transform.rotation = tf2::toMsg(imu_link_rotation);
        //by xwqf end

        stamped_transforms.push_back(stamped_transform);

        if(!imu_msg_.header.stamp.isZero())
        ...
        tf2::Quaternion imu_link_rotation;
        imu_link_rotation.setRPY(roll, pitch, yaw1);
        // tf2::Quaternion yaw_rotation;
        // yaw_rotation.setRPY(0.0, 0.0, yaw1);
        // imu_link_rotation = yaw_rotation * imu_link_rotation;
        stamped_transform.transform.rotation = tf2::toMsg(imu_link_rotation);
...
//        LOG(INFO) << stamped_transform.transform.translation << "  " << stamped_transform.transform.rotation;
        if(!imu_msg_.header.stamp.isZero())
