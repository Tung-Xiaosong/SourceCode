...
namespace lslidar_driver {
    ...
    bool lslidarDriver::loadParameters() {
        ...
        pnh.param("filter_angle_file", filter_angle_file, std::string(""));
...
/*                    float h_angle =
    ...
    bool lslidarC16Driver::initialize() {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            // ROS_INFO("aa=%d,bb=%d",filter_angle[i][0],filter_angle[i][1]);
        }
    ...
    bool lslidarC16Driver::poll(void) {  // Allocate a new shared pointer for zero-copy sharing with other nodelets.
        ...
        for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
            ...
            if (firings.azimuth[fir_idx] >= filter_angle[remapped_scan_idx][0] &&
                firings.azimuth[fir_idx] <= filter_angle[remapped_scan_idx][1]) { continue; }
            ...
            if (coordinate_opt) {
                int tmp_idx = 1468 - firings.azimuth[fir_idx] < 0 ? 1468 - firings.azimuth[fir_idx] + 36000 : 1468 -
                ...
                int tmp_idx = firings.azimuth[fir_idx] - 1468 < 0 ? firings.azimuth[fir_idx] - 1468 + 36000 :
                              firings.azimuth[fir_idx] - 1468;
...
//                    }
        ...
        for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
            ...
            double x_coord, y_coord, z_coord;
            if (coordinate_opt) {
                int tmp_idx = 1298 - firings.azimuth[fir_idx] < 0 ? 1298 - firings.azimuth[fir_idx] + 36000 : 1298 -
                                                                                                              firings.azimuth[fir_idx];
                ...
                //Y-axis correspondence 0 degree
                int tmp_idx = firings.azimuth[fir_idx] - 1298 < 0 ? firings.azimuth[fir_idx] - 1298 + 36000 :
                              firings.azimuth[fir_idx] - 1298;
