...
int CameraPublisher::start()
{
    ...
    std::string config_path = ros::package::getPath("ascamera_listener");

    // std::string config_path;
    // nh->param<std::string>("confiPath", config_path, "");
    config_path += "/configurationfiles";
...
int CameraPublisher::initLaunchParams()
{
    ...
    nh->param(node_name + "/depth_width", m_launch_param.set_depth_width, -1);
    nh->param(node_name + "/depth_height", m_launch_param.set_depth_height, -1);
    nh->param(node_name + "/rgb_width", m_launch_param.set_rgb_width, -1);
    nh->param(node_name + "/rgb_height", m_launch_param.set_rgb_height, -1);
    nh->param(node_name + "/ir_width", m_launch_param.set_ir_width, -1);
    nh->param(node_name + "/ir_height", m_launch_param.set_ir_height, -1);
    nh->param(node_name + "/peak_width", m_launch_param.set_peak_width, -1);
    nh->param(node_name + "/peak_height", m_launch_param.set_peak_height, -1);
    nh->param(node_name + "/fps", m_launch_param.set_fps, -1);
    nh->param(node_name + "/usb_bus_no", m_launch_param.usb_bus_num, -1);
    std::string usb_port_nums;
    nh->param<std::string>(node_name + "/usb_path", usb_port_nums, "null");
    strncpy(m_launch_param.usb_port_nums, usb_port_nums.c_str(),
            std::min(sizeof(m_launch_param.usb_port_nums), usb_port_nums.length()));
    nh->param(node_name + "/color_pcl", m_launch_param.color_pcl, false);
    nh->param(node_name + "/pub_mono8", m_launch_param.pub_mono8, false);
