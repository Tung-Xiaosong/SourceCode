...
void VelocitySmoother::velocityCB(const geometry_msgs::TwistStamped::ConstPtr& msg)
{
  ...
  locker.lock();
  target_vel.linear.x  =
      msg->twist.linear.x  > 0.0 ? std::min(msg->twist.linear.x,  speed_lim_v) : std::max(msg->twist.linear.x,  -speed_lim_v);
  target_vel.angular.z =
      msg->twist.angular.z > 0.0 ? std::min(msg->twist.angular.z, speed_lim_w) : std::max(msg->twist.angular.z, -speed_lim_w);
  locker.unlock();
}
...
//DGT
void VelocitySmoother::DgtChassisCB(const yhs_msgs::DgtChassisInfoFb::ConstPtr& msg)
{
  is_joy_control = msg->io_fb.io_fb_joypad_ctrl;
}

//FW
void VelocitySmoother::FwChassisCB(const yhs_msgs::FwChassisInfoFb::ConstPtr& msg)
{
  is_joy_control = msg->io_fb.io_fb_joypad_ctrl;
}

//MK
void VelocitySmoother::MkChassisCB(const yhs_msgs::MkChassisInfoFb::ConstPtr& msg)
{
  is_joy_control = msg->ctrl_fb.ctrl_fb_mode;
}

//FR
void VelocitySmoother::FrChassisCB(const yhs_msgs::FrChassisInfoFb::ConstPtr& msg)
{
  is_joy_control = msg->ctrl_fb.ctrl_fb_mode;
}
...
void VelocitySmoother::spin()
{
  ...
  while (! shutdown_req && ros::ok())
  {
    ...
    //没有接收到速度，发布0
    if( (ros::Time::now() - last_cb_time).toSec() > 0.2)
    {
      geometry_msgs::TwistStamped cmd_vel_pub;
      cmd_vel_pub.header.stamp = ros::Time::now();
      cmd_vel_pub.twist = ZERO_VEL_COMMAND;
      smooth_vel_pub.publish(cmd_vel_pub);
    }
    ...
    if ( ((target_vel.linear.x  != last_cmd_vel.linear.x) ||
        (target_vel.angular.z != last_cmd_vel.angular.z)) && !is_joy_control)
    {
      ...
      if(std::abs(cmd_vel->linear.x) > 0 && std::abs(cmd_vel->linear.x) < 0.1)
      {
        cmd_vel->linear.x = sign(v_inc) * 0.1;
      }
...
bool VelocitySmoother::init(ros::NodeHandle& nh)
{
  ...
  // Optional parameters
  int feedback;
  nh.param("/velocity_smoother/frequency",      frequency,     50.0);
  nh.param("/velocity_smoother/quiet",          quiet,         quiet);
  nh.param("/velocity_smoother/decel_factor",   decel_factor,   1.0);
  nh.param("/velocity_smoother/robot_feedback", feedback, (int)NONE);
  ...
  if ((nh.getParam("/velocity_smoother/speed_lim_v", speed_lim_v) == false) ||
      (nh.getParam("/velocity_smoother/speed_lim_w", speed_lim_w) == false))
  {
    ROS_ERROR("Missing velocity limit parameter(s)");
    return false;
  }

  if ((nh.getParam("/velocity_smoother/accel_lim_v", accel_lim_v) == false) ||
      (nh.getParam("/velocity_smoother/accel_lim_w", accel_lim_w) == false))
  {
  ...
  dynamic_reconfigure_server = new dynamic_reconfigure::Server<yocs_velocity_smoother::paramsConfig>(ros::NodeHandle("~"));
  dynamic_reconfigure_server->setCallback(dynamic_reconfigure_callback);

  
  if(chassis_type == "DGT")
  {
    dgt_chassis_info_sub = nh.subscribe<yhs_msgs::DgtChassisInfoFb>("chassis_info_fb", 10, &VelocitySmoother::DgtChassisCB,this);  
  }
  else if(chassis_type == "FW")
  {
    fw_chassis_info_sub = nh.subscribe<yhs_msgs::FwChassisInfoFb>("chassis_info_fb", 10, &VelocitySmoother::FwChassisCB,this); 
  }
  else if(chassis_type == "MK")
  {
    mk_chassis_info_sub = nh.subscribe<yhs_msgs::MkChassisInfoFb>("chassis_info_fb", 10, &VelocitySmoother::MkChassisCB,this); 
  }
  else if(chassis_type == "FR")
  {
    fr_chassis_info_sub = nh.subscribe<yhs_msgs::FrChassisInfoFb>("chassis_info_fb", 10, &VelocitySmoother::FrChassisCB,this); 
  }
  else
  {
  }
  ...
  raw_in_vel_sub  = nh.subscribe("cmd_vel",   1, &VelocitySmoother::velocityCB, this);
  smooth_vel_pub  = nh.advertise <geometry_msgs::TwistStamped> ("smoother_cmd_vel", 1);
  return true;
}
