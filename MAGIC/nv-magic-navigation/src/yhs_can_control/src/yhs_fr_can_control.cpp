...
namespace can_control
{
  ...
  FrCanControl::FrCanControl()
  {
    ...
    ctrl_cmd_sub_ = nh_.subscribe<yhs_msgs::FrCtrlCmd>("ctrl_cmd", 5, &FrCanControl::CtrlCmdCallBack, this);
  	io_cmd_sub_ = nh_.subscribe<yhs_msgs::FrIoCmd>("io_cmd", 5, &FrCanControl::IoCmdCallBack, this);
  	cmd_sub_ = nh_.subscribe<geometry_msgs::TwistStamped>("smoother_cmd_vel", 5, &FrCanControl::CmdCallBack, this);
  	
  	imu_sub_ = nh_.subscribe<sensor_msgs::Imu>("imu_data", 5, &FrCanControl::ImuDataCallBack, this);
  ...
  void FrCanControl::IoCmdCallBack(const yhs_msgs::FrIoCmd::ConstPtr& io_cmd_msg)
  {
  ...
  void FrCanControl::CtrlCmdCallBack(const yhs_msgs::FrCtrlCmd::ConstPtr& ctrl_cmd_msg)
  {
  ...
  void FrCanControl::CmdCallBack(const geometry_msgs::TwistStamped::ConstPtr& cmd_msg)
  {
		auto ctrl_cmd_msg = boost::make_shared<yhs_msgs::FrCtrlCmd>();
		...
		ctrl_cmd_msg->ctrl_cmd_gear = cmd_msg->twist.linear.x > 0 ? 4 : 2;
		ctrl_cmd_msg->ctrl_cmd_brake = 0.0;

		CtrlCmdCallBack(ctrl_cmd_msg);
  }
  
  void FrCanControl::RecvData()
  {
  	ros::Rate loop(100);
		...
		while (ros::ok())
		{
			...
			if(read(can_socket_, &recv_frames_, sizeof(recv_frames_)) >= 0)
			{
				switch (recv_frames_.can_id)
				{
					...
					case 0x98C4D7EF:
					{
						yhs_msgs::FrLrWheelFb msg;
					
						msg.lr_wheel_fb_velocity = static_cast<float>(static_cast<float>(recv_frames_.data[1] << 8 | recv_frames_.data[0])) / 1000;

						msg.lr_wheel_fb_pulse = static_cast<int>(recv_frames_.data[5] << 24 | recv_frames_.data[4] << 16 | recv_frames_.data[3] << 8 | recv_frames_.data[2]);
					...
					case 0x98C4D8EF:
					{
						yhs_msgs::FrRrWheelFb msg;
					
						msg.rr_wheel_fb_velocity = static_cast<float>(static_cast<short>(recv_frames_.data[1] << 8 | recv_frames_.data[0])) / 1000;

						msg.rr_wheel_fb_pulse = static_cast<int>(recv_frames_.data[5] << 24 | recv_frames_.data[4] << 16 | recv_frames_.data[3] << 8 | recv_frames_.data[2]);
					...
					case 0x98C4DCEF:
					{
						yhs_msgs::FrDriveMcuEcodeFb msg;
					
						msg.drive_fb_mcuecode = static_cast<int>(recv_frames_.data[3] << 24 | recv_frames_.data[2] << 16 | recv_frames_.data[1] << 8 | recv_frames_.data[0]); 
  ...
  void FrCanControl::ImuDataCallBack(const sensor_msgs::Imu::ConstPtr& imu_data_msg)
  {
  ...
  void FrCanControl::OdomPub(const float velocity,const float steering)
  {
  	static double x = 0.0;
  	static double y = 0.0;
...
//  	th += delta_th;
	...
	bool FrCanControl::Run() 
	{
		...
		thread_ = boost::thread(&FrCanControl::RecvData, this);
