...
//它并不包含main()函数，因为它是作为一个插件被加载到其他ROS节点中运行的。
...
//    allow_reversing_ = false;
  ...
  //减速到的最小速度
  nh.param<double>("min_approach_linear_velocity", min_approach_linear_velocity_, 0.15);
  ...
  //目标点距离参数  
  nh.param<double>("goal_dist_tol", goal_dist_tol_, 0.04);
                                                  ...
                                                  std::string &message)
{
  ...
  //判断是否到达目标点
  if(robot_to_goal_dist < goal_dist_tol_ && fabs(delta_orient) < 0.02)
  {
    goal_reached_ = true;
    return true;
  }
...
bool RegulatedPurePursuitController::setNewGoal()
{
