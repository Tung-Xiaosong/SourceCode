...
//by add
void VelocitySmoother::chassisCB(const chassis_msg_type::ConstPtr& msg)
{
#if CAR_TYPE == 3
  is_joy_control = msg->ctrl_fb.ctrl_fb_mode == 1 ? 1 : 0;
#elif CAR_TYPE == 4
  is_joy_control = ! msg->veh_diag_fb.veh_fb_auto_io_can_cmd;
#else
...
bool VelocitySmoother::init(ros::NodeHandle& nh)
{
  ...
  //by add
  chassis_info_sub = nh.subscribe<chassis_msg_type>("chassis_info_fb", 10, &VelocitySmoother::chassisCB,this);
  ...
  smooth_vel_pub  = nh.advertise <geometry_msgs::TwistStamped> ("smooth_cmd_vel", 1);
  return true;
}
