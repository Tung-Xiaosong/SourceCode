...
//	send_frame.can_id = 0x98C4D2D0;//dxs change 0x98C4D1D0
  send_frame.can_id = 0x98C4D1D0;
	send_frame.can_dlc = 8;
...
void CanControl::recvData()
{
    ...
    if(!wait_for_can_frame()) continue;
		...
		//ultrasonic 
		static yhs_msgs::Ultrasonic ultra_msg;
		case 0x98C4E8EF:
		{
			ultra_msg.left = (unsigned short)((recv_frames_[0].data[1] & 0x0f) << 8 | recv_frames_[0].data[0]);
			ultra_msg.front = (unsigned short)(recv_frames_[0].data[2]  << 4 | ((recv_frames_[0].data[1] & 0xf0) >> 4));

			ultra_msg.right = (unsigned short)((recv_frames_[0].data[4] & 0x0f) << 8 | recv_frames_[0].data[3]);
			ultra_msg.rear = (unsigned short)(recv_frames_[0].data[5]  << 4 | ((recv_frames_[0].data[4] & 0xf0) >> 4));
			break;
		}

		case 0x98C4E9EF:
		{
			ultra_msg.right = (unsigned short)((recv_frames_[0].data[1] & 0x0f) << 8 | recv_frames_[0].data[0]);
			ultra_msg.rear = (unsigned short)(recv_frames_[0].data[2]  << 4 | ((recv_frames_[0].data[1] & 0xf0) >> 4));

			ultra_msg.left = (unsigned short)((recv_frames_[0].data[4] & 0x0f) << 8 | recv_frames_[0].data[3]);
			ultra_msg.front = (unsigned short)(recv_frames_[0].data[5]  << 4 | ((recv_frames_[0].data[4] & 0xf0) >> 4));
			...
			scan.ranges[32] = ((double)ultra_msg.rear / 1000 > 0.5 || ultra_msg.rear == 0) ? std::numeric_limits<float>::infinity() : (double)ultra_msg.rear / 1000 + 0.87;
			
			scan.ranges[8] = ((double)ultra_msg.left / 1000 > 0.5 || ultra_msg.left == 0) ? std::numeric_limits<float>::infinity() : (double)ultra_msg.left / 1000 + 0.87;

			scan.ranges[24] = ((double)ultra_msg.right / 1000 > 0.5 || ultra_msg.right == 0) ? std::numeric_limits<float>::infinity() : (double)ultra_msg.right / 1000 + 0.67;
			
			scan.ranges[16] = ((double)ultra_msg.front / 1000 > 0.5 || ultra_msg.front == 0) ? std::numeric_limits<float>::infinity() : (double)ultra_msg.front / 1000 + 0.87;
...
void CanControl::imu_cmdCallBack(const sensor_msgs::Imu msg)//dxs_add
{
...
void CanControl::io_cmdCallBack(const yhs_msgs::MkIoCmd::ConstPtr & io_cmd_msg)
{
  yhs_msgs::MkIoCmd msg = *io_cmd_msg;
  static unsigned char count = 0;
...
void CanControl::ctrl_cmdCallBack(const yhs_msgs::MkCtrlCmd::ConstPtr & ctrl_cmd_msg)
{
  yhs_msgs::MkCtrlCmd msg = *ctrl_cmd_msg;
  const unsigned short vel = msg.ctrl_cmd_velocity * 1000;
  const short angular = msg.ctrl_cmd_steering * 100;
  const unsigned char gear = msg.ctrl_cmd_gear;
  const unsigned char brake = msg.ctrl_cmd_gear;
...
void CanControl::cmdCallBack(const geometry_msgs::TwistStamped::ConstPtr & cmd_msg)
{
  yhs_msgs::MkCtrlCmd::Ptr ctrl_cmd_msg = boost::make_shared<yhs_msgs::MkCtrlCmd>();

  ctrl_cmd_msg->ctrl_cmd_velocity = fabs(cmd_msg->twist.linear.x);
  ctrl_cmd_msg->ctrl_cmd_steering = cmd_msg->twist.angular.z / 3.14 * 180;
  ctrl_cmd_msg->ctrl_cmd_gear = cmd_msg->twist.linear.x < 0 ? 2 : 4;

  ctrl_cmdCallBack(ctrl_cmd_msg);
}
...
void CanControl::recvData()
{
  ...
  yhs_msgs::MkChassisInfoFb chassis_info_msg;

  while (ros::ok())
  {
    if(!wait_for_can_frame()) continue;
    
    if(read(dev_handler_, &recv_frame, sizeof(recv_frame)) >= 0)
    {
      switch (recv_frame.can_id)
      {
        case 0x98C4D2EF:
        {
          ...
          msg.ctrl_fb_velocity = static_cast<float>(static_cast<unsigned short>((recv_frame.data[2] & 0x0f) << 12 | recv_frame.data[1] << 4 | (recv_frame.data[0] & 0xf0) >> 4)) / 1000;
          ...
          msg.ctrl_fb_brake = (recv_frame.data[4] & 0x30) >> 4;
        
          msg.ctrl_fb_mode = (recv_frame.data[4] & 0xc0) >> 6;
        ...
        case 0x98C4E2EF:
        {
          ...
          msg.bms_flag_info_soc_low_protection = (0x80 & recv_frame.data[2]) != 0;
        ...
        case 0x98C4DCEF:
        {
          yhs_msgs::MkDriveMcuEcodeFb msg;
        ...
        case 0x98C4EAEF:
        {
          ...
          msg.veh_fb_rd_rv_mcu_fault = (recv_frame.data[5] & 0x0f << 2) | (recv_frame.data[4] >> 6);
...
void CanControl::odomPub(const float velocity, const float steering)
{
	...
	double vx = velocity;
	double vth = vx * tan(steering) / 0.6;
...
#elif CAR_TYPE == 4
  ctrl_cmd_sub_ = nh_.subscribe<yhs_msgs::MkCtrlCmd>("ctrl_cmd", 5, &CanControl::ctrl_cmdCallBack, this);
	io_cmd_sub_ = nh_.subscribe<yhs_msgs::MkIoCmd>("io_cmd", 5, &CanControl::io_cmdCallBack, this);
	cmd_sub_ = nh_.subscribe<geometry_msgs::TwistStamped>("smoother_cmd_vel", 5, &CanControl::cmdCallBack, this);
