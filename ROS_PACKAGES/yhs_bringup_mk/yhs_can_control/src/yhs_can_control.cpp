			...
			for(int j=0;j<1;j++)
			{
				...
				switch (recv_frames_[0].can_id)
				{
					...
					case 0x98C4D2EF:
					{
						...
						msg.ctrl_fb_gear = 0x0f & recv_frames_[0].data[0];//data[0~7],一个recv_frames_(一帧canID)中有8个字节(byte),每一个字节有8个位(bit),位用0或1表示,data[0]代表第一个字节
...
/*					static yhs_can_msgs::ultrasonic ulmsg;
					case 0x98C4E8EF:
					{
						ulmsg.left = (unsigned short)((recv_frames_[0].data[1] & 0x0f) << 8 | recv_frames_[0].data[0]);
						ulmsg.front = (unsigned short)(recv_frames_[0].data[2]  << 4 | ((recv_frames_[0].data[1] & 0xf0) >> 4));

						ulmsg.right = (unsigned short)((recv_frames_[0].data[4] & 0x0f) << 8 | recv_frames_[0].data[3]);
						ulmsg.rear = (unsigned short)(recv_frames_[0].data[5]  << 4 | ((recv_frames_[0].data[4] & 0xf0) >> 4));
						break;
					}

					case 0x98C4E9EF:
					{
						ulmsg.right = (unsigned short)((recv_frames_[0].data[1] & 0x0f) << 8 | recv_frames_[0].data[0]);
						ulmsg.rear = (unsigned short)(recv_frames_[0].data[2]  << 4 | ((recv_frames_[0].data[1] & 0xf0) >> 4));

						ulmsg.left = (unsigned short)((recv_frames_[0].data[4] & 0x0f) << 8 | recv_frames_[0].data[3]);
						ulmsg.front = (unsigned short)(recv_frames_[0].data[5]  << 4 | ((recv_frames_[0].data[4] & 0xf0) >> 4));
						...
						scan.ranges[32] = ((double)ulmsg.rear / 1000 > 0.5 || ulmsg.rear == 0) ? std::numeric_limits<float>::infinity() : (double)ulmsg.rear / 1000 + 0.87;
						
            					scan.ranges[8] = ((double)ulmsg.left / 1000 > 0.5 || ulmsg.left == 0) ? std::numeric_limits<float>::infinity() : (double)ulmsg.left / 1000 + 0.87;

						scan.ranges[24] = ((double)ulmsg.right / 1000 > 0.5 || ulmsg.right == 0) ? std::numeric_limits<float>::infinity() : (double)ulmsg.right / 1000 + 0.67;
						
            					scan.ranges[16] = ((double)ulmsg.front / 1000 > 0.5 || ulmsg.front == 0) ? std::numeric_limits<float>::infinity() : (double)ulmsg.front / 1000 + 0.87;
...
void CanControl::odomPub(float velocity,float steering)//dxs_add
{
	static double x = 0.0;
	static double y = 0.0;
...
void CanControl::cmdCallBack(const geometry_msgs::Twist msg)
{
	...
	memset(sendData_u_vel,0,8);//用于初始化一个字节数组,将数组 sendData_u_vel 的前8个字节（即数组的整个大小）都设置为 0
...
void CanControl::run()
{
	...
	imu_sub_ = nh_.subscribe<sensor_msgs::Imu>("imu_data", 5, &CanControl::imu_cmdCallBack, this);//dxs_add
	...
	scan_pub_ = nh_.advertise<sensor_msgs::LaserScan>("scan3",5);//dxs
	ultrasonic_pub_ = nh_.advertise<yhs_can_msgs::ultrasonic>("ultrasonic",5);//dxs
